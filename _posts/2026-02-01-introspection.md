...
<!-- A model is self-consistent if it can both produce self-descriptions of its external behaviors or internal mechanisms, and exhibit the corresponding behaviors or mechanisms they describe.
<!-- agree with its behaviors or internal mechanisms, and vice versa. -->
<!-- Self-descriptions may refer either to external behaviors or to internal mechanisms. -->

<!-- Behavioral descriptions concern observable outputs of the model—such as the exact tokens it generates in a given context, patterns shared across many generations, or systematic differences between outputs under controlled input changes. A behavioral description is consistent when it accurately characterizes these observable behaviors across a range of contexts. -->
<!-- Mechanistic descriptions concern internal properties of the model—such as model features, circuits, or parameters. A feature description is consistent if it correctly predicts when that feature will activate across diverse inputs. A circuit description is consistent if intervening on components of the circuit causes downstream activations or behaviors to change in the predicted way. -->

<!-- If the same model can both produce these descriptions and exhibit the corresponding behaviors or mechanisms they describe, we say the model is self-consistent. -->
...
While general versions of causality across all intervention types may be unenforceable, it is nonetheless a desirable property for the aforementioned reasons.[^3]
...
This prevents M1 from simply simulating itself and making post-hoc observations, as M2 would be able to obtain equally accurate explanations by running the same M1 simulations.[^4]
...
At the beginning of this blog post, I opened with the possibility that introspection could deliver a “ChatGPT moment” for interpretability. While this is an ambitious claim that remains to be proven, I think it’s safe to say that introspection is at least a step in the right direction: it moves interpretability toward being more interactive, accessible, and scalable. Furthermore, we now have some [early](https://arxiv.org/abs/2511.08579) [empirical](https://arxiv.org/abs/2510.05092) [evidence](https://arxiv.org/abs/2505.17120) indicating that training for introspection is [at](https://arxiv.org/abs/2501.11120) [least](https://arxiv.org/abs/2410.13787) [possible](https://arxiv.org/abs/2512.08093) in narrow tasks.
